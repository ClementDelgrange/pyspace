...
def generate_mnt(t_in, dif, n):
    """
    Génération d'une grille régulière de d'altitudes.

    La méthode s'appuie sur l'altitude des 4 extrémités de la grille et calcule
    les altitudes des points intermédiaires à partir de la moyenne des points
    connus à laquelle un bruit est ajouté.
    Le processus est répété sur les sous-carrés autant de fois que souhaité.

    :param t_in: tableau des 4 points en entrée [[pno, pne], [pso, pse]]
    :param dif: valeur de bruit autorisée dans le calcul des altitudes
    :param n: nombre de fois où l'opération d'évaluation du mnt est répétée
    :return: tableau des points en sortie
    """
    ...
    if n > 1:
        ...
        t1_out = generate_mnt(t1, dif, n-1)
        t2_out = generate_mnt(t2, dif, n-1)
        t3_out = generate_mnt(t3, dif, n-1)
        t4_out = generate_mnt(t4, dif, n-1)
...
def eval_carre(t, dif):
    """
    Génération aléatoire des altitudes d'un carré dont on connait les quatre
    extrémités.
...
def fusion_tab(t1, t2, t3, t4):
    ...
    Les tableaux forment un carré. On prend la moyenne des extrémités communes
    aux tableaux. Pour les autres valeurs, on copie juste la valeur du tableau
    initial.
    ...
    :param t1: tableau inférieur gauche
    :param t2: tableau inférieur droit
    :param t3: tableau supérieur gauche
    :param t4: tableau supérieur droit
    :return: tableau issu de la fusion
    """
    n = len(t1)
